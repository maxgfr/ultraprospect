...
describe("scorePair", () => {
  ...
  it("REFUSES a pair with no identity agreement, however close", () => {
    ...
    const s = scorePair(poi({ name: "Le Bistrot" }), rec({ legalName: "SOCIETE GENERALE" }));
  ...
  it("sends an address-only pair to ADJUDICATION rather than merging it", () => {
    ...
    const r = rec({ legalName: "AUX BARREZIENS", address: { numero: "12", typeVoie: "AVENUE", libelleVoie: "DE PARIS" } });
  ...
  it("MERGES an address match that even weakly agrees on the name", () => {
    ...
    const r = rec({ legalName: "MAISON 123", address: { numero: "12", typeVoie: "AVENUE", libelleVoie: "DE PARIS" } });
    const s = scorePair(p, r);
    expect(s.parts.address).toBe(1);
    ...
    const alone = scorePair(poi({ name: "Maison 1 2 3" }), rec({ legalName: "MAISON 123" }));
  ...
  it("uses the brand tag against the register's enseigne", () => {
    ...
    const r = rec({ legalName: "SOMEDIS", tradingNames: ["CARREFOUR CITY"] });
    const s = scorePair(p, r);
  ...
  it("reports which register name produced the score", () => {
    ...
    const r = rec({ legalName: "COMMUNE DE VINCENNES", tradingNames: ["CRECHE BURGEAT"] });
...
describe("matchLanes", () => {
  it("pairs one-to-one, best first", () => {
    ...
    // Keyed by the fully-qualified record key: two connectors can serve one
    // country and their identifier spaces do not overlap.
    expect(merged.get("fr-sirene:A")).toMatchObject({ osmId: "n1", by: "name" });
    expect(merged.get("fr-sirene:A")!.score).toBeGreaterThanOrEqual(MERGE_HIGH);
  });

  it("routes the middle band to the agent instead of deciding", () => {
    const p = poi({ name: "Maison 1 2 3" });
    const r = rec({ legalName: "MAISON 123" });
    const s = scorePair(p, r);
...
describe("buildMatchTodo", () => {
  it("sorts strongest first so the agent works down an easing list", () => {
    const todo = buildMatchTodo([
      { osmId: "a", connectorId: "fr-sirene", registryId: "A", score: 0.5, parts: { distance: 0, name: 0, enseigne: 0, address: 0 }, distanceM: 10 },
      { osmId: "b", connectorId: "fr-sirene", registryId: "B", score: 0.7, parts: { distance: 0, name: 0, enseigne: 0, address: 0 }, distanceM: 10 },
    ]);
...
describe("applyVerdicts", () => {
  function places(): Place[] {
    return [
      ...
      },
      {
        ...
        sources: ["registry"],
        registry: rec({ establishmentId: "A", address: { commune: "VINCENNES" } }),
  ...
  it("merges a confirmed pair into the OSM entity and removes the duplicate", () => {
    const list = places();
    const result = applyVerdicts(list, [{ osmId: "n1", registryId: "A", connectorId: "fr-sirene", merge: true }]);
    expect(result.merged).toBe(1);
    expect(list).toHaveLength(1);
    expect(list[0]!.id).toBe("osm:n1");
    expect(list[0]!.registry?.establishmentId).toBe("A");
    expect(list[0]!.sources).toEqual(["osm", "registry"]);
  ...
  it("changes nothing for a keep-apart verdict", () => {
    ...
    const list = places();
    const result = applyVerdicts(list, [{ osmId: "n1", registryId: "A", connectorId: "fr-sirene", merge: false }]);
  ...
  it("reports a verdict naming a pair this run does not have", () => {
    const list = places();
    const result = applyVerdicts(list, [{ osmId: "n999", registryId: "ZZZ", connectorId: "fr-sirene", merge: true }]);
    expect(result.merged).toBe(0);
