#!/usr/bin/env node
// Mirror the source-of-truth bundle (scripts/ultraprospect.mjs, produced by tsup)
// byte-for-byte into the skill package. The skill ships standalone — `npx
// skills add` copies the skill directory (skills/ultraprospect/), so the engine
...
const source = join(root, "scripts", "ultraprospect.mjs");
const targets = [join(root, "skills", "ultraprospect", "scripts", "ultraprospect.mjs")];
